...
class DirectVoxGO(torch.nn.Module):
    ...
    def activate_density(self, density, interval=None):
        ...
        interval=float(interval.data)#修改部分
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, is_train=False, near_far=False, **render_kwargs):
        ...
        self.voxel_size.to(rng.device)
        step = stepsize * self.voxel_size * rng
        self.voxel_size.to(rng.device)
        step.to(rays_d.device)
        interpx = (t_min[...,None] + step/rays_d.norm(dim=-1,keepdim=True))
        rays_pts = rays_o[...,None,:] + rays_d[...,None,:] * interpx[...,None]
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        ######修改部分
        N_samples = int(np.linalg.norm(np.array(self.density.shape[2:])+1) / render_kwargs['stepsize']) + 1
        # 2. determine the two end-points of ray bbox intersection
        vec = torch.where(rays_d==0, torch.full_like(rays_d, 1e-6), rays_d)
        rate_a = (self.xyz_max - rays_o) / vec
        rate_b = (self.xyz_min - rays_o) / vec
        ...
        t_max = torch.maximum(rate_a, rate_b).amin(-1).clamp(min=render_kwargs['near'], max=render_kwargs['far'])
        # 3. check wheter a raw intersect the bbox or not
        mask_outbbox = (t_max <= t_min)
        # 4. sample points on each ray
        rng = torch.arange(N_samples)[None].float()
        ...
        if is_train:
            rng = rng.repeat(rays_d.shape[-2],1)
            rng += torch.rand_like(rng[:,[0]])
        voxel_size=float(self.voxel_size.data)
        #voxel_size.to(rng.device)
        step = render_kwargs['stepsize'] * voxel_size * rng
