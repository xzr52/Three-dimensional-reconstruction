...
class Voxurf(torch.nn.Module):
    ...
    def __init__(self, xyz_min, xyz_max,
        ...
        self.gradient=torch.nn.Parameter(torch.zeros([1, 3, *self.world_size]))#修改部分
    ...
    def density_total_variation(self, sdf_tv=0, smooth_grad_tv=0, sdf_thrd=0.999,ori_gradient=None):
        tv = 0
        voxel_size=float(self.voxel_size)

        self.nonempty_mask = self.nonempty_mask.clone().detach().to(self.gradient.data.device)#修改部分
        if sdf_tv > 0:
            self.nonempty_mask = self.nonempty_mask.to(self.sdf.grid.device)
            tv += total_variation(self.sdf.grid, self.nonempty_mask) / 2 / voxel_size * sdf_tv
        if smooth_grad_tv > 0:
            self.tv_smooth_conv.to(self.gradient.data.device)#修改部分
            smooth_tv_error = (self.tv_smooth_conv(self.gradient.data.permute(1,0,2,3,4)).detach() - self.gradient.data.permute(1,0,2,3,4))
            smooth_tv_error = smooth_tv_error[self.nonempty_mask.repeat(3,1,1,1,1)] ** 2
            """ smooth_tv_error = (self.tv_smooth_conv(ori_gradient.permute(1,0,2,3,4)).detach() - ori_gradient.permute(1,0,2,3,4))
            smooth_tv_error = smooth_tv_error[self.nonempty_mask.repeat(3,1,1,1,1)] ** 2 """
            tv += smooth_tv_error.mean() * smooth_grad_tv
        self.tv_smooth_conv.to(self.sdf.grid.device)
        return tv
    ...
    def neus_sdf_gradient(self, mode=None, sdf=None):
        voxel_size=float(self.voxel_size)#修改部分
        if sdf is None:
            sdf = self.sdf.grid
        if mode is None:
            mode = self.grad_mode
        if mode == 'interpolate':
            ...
            gradient[:,0,1:-1,:,:] = (sdf[:,0,2:,:,:] - sdf[:,0,:-2,:,:]) / 2 / voxel_size
            gradient[:,1,:,1:-1,:] = (sdf[:,0,:,2:,:] - sdf[:,0,:,:-2,:]) / 2 / voxel_size
            gradient[:,2,:,:,1:-1] = (sdf[:,0,:,:,2:] - sdf[:,0,:,:,:-2]) / 2 / voxel_size
        elif mode == 'grad_conv':
            ...
            gradient = torch.zeros([1, 3] + [*self.sdf.grid.shape[-3:]]).to(self.sdf.grid.device)
            gradient[:,0,:-1,:,:] = (sdf[:,0,1:,:,:] - sdf[:,0,:-1,:,:]) / voxel_size
            gradient[:,1,:,:-1,:] = (sdf[:,0,:,1:,:] - sdf[:,0,:,:-1,:]) / voxel_size
            gradient[:,2,:,:,:-1] = (sdf[:,0,:,:,1:] - sdf[:,0,:,:,:-1]) / voxel_size
        else:
            raise NotImplementedError
        return gradient
    ...
    def neus_alpha_from_sdf_scatter(self, viewdirs, ray_id, dist, sdf, gradients, global_step,
        ...
        if use_mid:
            ...
            estimated_next_sdf = sdf + iter_cos * dist * 0.5 # (M, 1)#修改部分
            estimated_prev_sdf = sdf - iter_cos * dist * 0.5 # (M, 1)
        else:
    ...
    def grid_sampler(self, xyz, *grids, mode=None, align_corners=True, smooth=False, displace=0.):
        ...
        if displace !=0:
            ind_norm[...,:] += displace * voxel_size
        # TODO: use `rearrange' to make it readable
        if smooth:
            grid = self.smooth_conv(grids[0])
        else:
            grid = grids[0]
        ret_lst = F.grid_sample(grid, ind_norm.to(grid.device), mode=mode, align_corners=align_corners
    ...
    def sample_ray_cuda(self, rays_o, rays_d, near, far, stepsize, maskout=True, use_bg=False, **render_kwargs):
        ...
        if not use_bg:
            ...
            stepdist=np.array([float(stepdist)])
            stepdist=torch.from_numpy(stepdist).to(rays_o.device)
        else:
    ...
    def forward(self, rays_o, rays_d, viewdirs,global_step=None, **render_kwargs):
        ...
        if not use_bg:
            ...
            stepdist =  render_kwargs['stepsize'] * self.voxel_size_bg
        ray_pts, mask_outbbox, ray_id, step_id, N_steps, t_min, t_max = render_utils_cuda.sample_pts_on_rays(
            rays_o, rays_d, self.xyz_min, self.xyz_max, render_kwargs['near'], far, stepdist)
        # correct the cuda output N_steps, which could have a bias of 1 randomly
        N_steps = ray_id.unique(return_counts=True)[1]
        maskout=True
        if maskout:
        ...
        ray_pts, ray_id, step_id, mask_outbbox, N_steps
        ...
        self.gradient.to(sdf.device)
        self.gradient.data= self.neus_sdf_gradient(sdf=self.sdf.grid)#修改部分
        #self.gradient.data.to(self.sdf.grid.device)
        gradient = self.grid_sampler(ray_pts, self.gradient)#修改部分
        dist = render_kwargs['stepsize'] * voxel_size
...
class MaskCache(nn.Module):
    ...
    def forward(self, xyz):
        ...
        ind_norm = ((xyz - self.xyz_min) / (self.xyz_max - self.xyz_min)).flip((-1,)) * 2 - 1
        ###修改部分
        act_shift=float(self.act_shift)
        voxel_size_ratio=float(self.voxel_size_ratio)
        if self.nearest:
...
def get_training_rays_in_maskcache_sampling(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, model, render_kwargs):
    ...
    for c2w, img, (H, W), K in zip(train_poses, rgb_tr_ori, HW, Ks):
        ...
        for i in range(0, img.shape[0], CHUNK):
            rays_pts, mask_outbbox, _ = model_module.sample_ray_ori(
                rays_o=rays_o[i:i+CHUNK], rays_d=rays_d[i:i+CHUNK], **render_kwargs)
            mask_outbbox[~mask_outbbox] |= (~model_module.mask_cache(rays_pts[~mask_outbbox]))
