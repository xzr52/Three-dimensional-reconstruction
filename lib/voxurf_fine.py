...
class Voxurf(torch.nn.Module):
    ...
    def init_sdf_from_sdf(self, sdf0=None, smooth=False, reduce=1., ksize=3, sigma=1., zero2neg=True):
        ...
        self.gradient.data = self.neus_sdf_gradient()
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        self.gradient =torch.nn.Parameter(
            F.interpolate(self.gradient.data, size=tuple(self.world_size), mode='trilinear', align_corners=True))
    ...
    def sample_sdfs(self, xyz, *grids, displace_list, mode='bilinear', align_corners=True, use_grad_norm=False):
        ...
        if diff.numel()==0:
            diff=diff.reshape(feat_diff.shape)
        grad = feat_diff / diff / voxel_size
    ...
    def sample_ray(self, rays_o, rays_d,is_train, near, far, stepsize, **render_kwargs):
...
class MaskCache(nn.Module):
    ...
    def forward(self, xyz):
        voxel_size_ratio=float(self.voxel_size_ratio)#修改部分
        shape = xyz.shape[:-1]
        xyz = xyz.reshape(1,1,1,-1,3)
        ind_norm = ((xyz - self.xyz_min) / (self.xyz_max - self.xyz_min)).flip((-1,)) * 2 - 1
        if self.nearest:
            density = F.grid_sample(self.density, ind_norm, align_corners=True, mode='nearest')
            alpha = 1 - torch.exp(-F.softplus(density + self.act_shift) * voxel_size_ratio)
        else:
