...
def config_parser():
    ...
    parser.add_argument('--config', default='/opt/data/private/nerf_mesh/Voxurf-main/configs/custom_e2e/fine.py',
    ...
    parser.add_argument("--render_test",default=True, action='store_true')
    ...
    parser.add_argument("-p", "--prefix", type=str, default="scene_16",
    ...
    parser.add_argument("--sdf_mode", type=str, default="voxurf_coarse")
    parser.add_argument("--scene", type=str, default='/scene_16')
    parser.add_argument("--no_dvgo_init", action='store_true')
    parser.add_argument("--run_dvgo_init",default=False, action='store_true')
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None, use_dvgo=False):
    ...
    if cfg_model.maskout_near_cam_vox:
        model_module.maskout_near_cam_vox(poses[i_train,:3,3], near)
    ...
    # init sdf
    if load_sdf_from:
        if hasattr(model_module, 'init_sdf_from_sdf'):
            ...
            sdf0 = utils.load_grid_data(model_module, cfg_train.load_sdf_from, name='sdf', return_raw=True)
            model_module.init_sdf_from_sdf(sdf0, smooth=False, reduce=sdf_reduce)
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = Model.get_training_rays_in_maskcache_sampling(
                ...
                ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y,
                model=model_module, render_kwargs=render_kwargs,
            )
    ...
    if cfg_train.pervoxel_lr:
        def per_voxel_init():
            cnt = model_module.voxel_count_views(
                rays_o_tr=rays_o_tr, rays_d_tr=rays_d_tr, imsz=imsz, near=near, far=far,
                stepsize=cfg_model.stepsize, downrate=cfg_train.pervoxel_lr_downrate,
                irregular_shape=data_dict['irregular_shape'])
            optimizer.set_pervoxel_lr(cnt)
            with torch.no_grad():
                model_module.density[cnt <= 2] = -100
        per_voxel_init()
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            if hasattr(model_module, 'num_voxels_bg'):
                model_module.scale_volume_grid(model_module.num_voxels * scale_ratio, model_module.num_voxels_bg * scale_ratio)
            else:
                model_module.scale_volume_grid(model_module.num_voxels * scale_ratio)
            optimizer = utils.create_optimizer_or_freeze_model(model, cfg_train, global_step=0)
        ...
        if global_step>cfg_train.tv_from and global_step<cfg_train.tv_end and global_step%cfg_train.tv_every==0:
            if cfg_train.weight_tv_density>0:
                ...
                if smooth_grad_tv > 0:
                    loss += (cfg_train.weight_tv_density * model_module.density_total_variation(sdf_tv=0, smooth_grad_tv=smooth_grad_tv)).to(loss.device)
                if getattr(cfg_train, 'ori_tv', False):
                    loss += cfg_train.weight_tv_density * model_module.density_total_variation(sdf_tv=sdf_tv, smooth_grad_tv=0)
                    weight_tv_k0 = getattr(cfg_train, 'weight_tv_k0')
                    if weight_tv_k0 > 0:
                        k0_tv_terms = getattr(cfg_train, 'k0_tv_terms', dict())
                        loss += cfg_train.weight_tv_k0 * model_module.k0_total_variation(**k0_tv_terms)
                    if getattr(tv_terms, 'bg_density_tv', 0):
                        loss += cfg_train.weight_tv_density * model_module.density_total_variation(sdf_tv=0, smooth_grad_tv=0, bg_density_tv=tv_terms['bg_density_tv'])
        if getattr(cfg_train, 'ori_tv', False) and cfg_train.get('weight_bg_tv_k0', 0) >0 and global_step>cfg_train.tv_from and global_step%cfg_train.tv_every==0 and global_step<cfg_train.tv_end:
            ...
            loss += cfg_train.get('weight_bg_tv_k0', 0) * model_module.bg_k0_total_variation(**bg_k0_tv_terms)
        ...
        if global_step>cfg_train.tv_from and global_step<cfg_train.tv_end and global_step%cfg_train.tv_every==0:
            if not getattr(cfg_train, 'ori_tv', False):
                if cfg_train.weight_tv_density>0:
                    ...
                    if sdf_tv > 0:
                        model_module.sdf_total_variation_add_grad(
        ...
        if global_step%args.i_print==0:
            ...
            logger.info(f'scene_rep_reconstruction ({stage}): iter {global_step:6d} / '
                        ...
                        #f' / s_val: {np.mean([t.cpu().numpy() for t in s_val_lst]):5.2g} / mask\%: {100*np.mean(mask_lst):1.2f} / bg_mask\%: {100*bg_mask_mean:1.2f} '
                        f'Eps: {eps_time_str}')
        ...
        if global_step == cfg_train.N_iters:
            torch.save({
                ...
                'model_kwargs': model_module.get_kwargs(),
                'MaskCache_kwargs': model_module.get_MaskCache_kwargs(),
        ...
        if global_step == cfg_train.N_iters and stage == 'surf' and 'fine' in args.sdf_mode:
            validate_mesh(model_module, 512, threshold=0.0, prefix="{}final".format(prefix), world_space=True,
...
def validate_mesh(model, resolution=128, threshold=0.0, prefix="", world_space=False,
    ...
    if extract_color:
        ...
        vertex_colors = (torch.cat(vertex_colors).cpu().detach().numpy() * 255.).astype( np.uint8)
